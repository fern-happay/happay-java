package com.happay.api.client.account.exceptions;
...
@JsonDeserialize(
    using = CreateAccountException.Deserializer.class
)
public final class CreateAccountException extends Exception {
  private final Value value;

  private int statusCode;

  private CreateAccountException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CreateAccountException other(Object unknownValue, int statusCode) {
    return new CreateAccountException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CreateAccountException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CreateAccountException> {
    @Override
    public CreateAccountException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CreateAccountException(value, statusCode);
    }
  }
}
